...
function load() {
	...
	function initReadMore() {
		const items = document.querySelectorAll('.item-reviews__text')
		if (items.length) {
			items.forEach(item => {
				item.style.height = ``
				setReadMore(item, item.offsetHeight > 100)
			})
		}
		function setReadMore(item, type) {
			const itemParent = item.parentElement
			item.style.height = type ? `100px` : ''

			if (type && !itemParent.querySelector('.item-reviews__more')) {
				itemParent.insertAdjacentHTML("beforeend", `<button class="item-reviews__more">Read more</button>`)
			}
			if (!type && itemParent.querySelector('.item-reviews__more')) {
				itemParent.querySelector('.item-reviews__more').remove()
			}
		}
	}
	...
	function documentActions(e) {
		...
		if (eventType === 'click') {
			...
			if (targetElement.closest('.item-reviews__more')) {
				const currentButton = targetElement.closest('.item-reviews__more')
				const item = currentButton.previousElementSibling

				if (currentButton.closest('[class*="--open"]')) {
					item.style.height = `100px`
					currentButton.innerHTML = 'Read more'
					setTimeout(() => {
						currentButton.classList.remove('item-reviews__more--open')
					}, 300)
				} else {
					currentButton.classList.add('item-reviews__more--open')
					item.style.height = ``
